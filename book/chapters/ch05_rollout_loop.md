...
上一章是 Megatron 侧。这一章是 SGLang 侧。它们并列是全书最重要的
两章——分别解释 slime "一条数据路径" 赌注在训练侧和推理侧怎么落地。
...
`PENDING / COMPLETED / TRUNCATED` 是常规生成路径；`ABORTED` 与
`FAILED` 是这套状态机最值得说的两个状态。
...
def _convert_samples_to_train_data(self, samples):
    train_data = {...}
    # 关键步骤：每个 rollout 的 total mask sum 复制给该 rollout 的每个 sample
